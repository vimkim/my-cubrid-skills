...
- Consult `my-cubrid-skills-create`, `writing-for-agents`, `skill-creator`, and this repository's AGENTS.md for implementation. Consult existing `cubrid-ci-analyze`, `cubrid-ci-trigger`, `cubrid-build`, focused regression runners, and `create-testcases` for the audit; use `research` for research tickets and `grilling` plus `domain-modeling` for human decisions.
